...
**Validator (`mosrac_s_validator.h`).** Raw readings are not trusted directly. The validator does two things.

- It requires `kStartupCount = 5` consecutive CRC-valid readings that agree within `kCpr/16` before transitioning from inactive to active, which rejects power-on garbage.
- It clears the active state when valid readings stop arriving, for example on a cable disconnect.
...
For questions about the broader (proprietary) actuator work, email ethanmathias@gmail.com.
